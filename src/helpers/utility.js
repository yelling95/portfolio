...
// 통화
export const renderCurrencyUnit = ({lang, number, unitText, unit = 1}) => {
  const renderContents = [convertThousandSeparator(number, 1) || '-', unitText]

  switch (lang) {
    case 'ko':
      renderContents[0] = convertThousandSeparator(number, unit, 0) || '-'
      break
    case 'en':
      renderContents.reverse()
      break
  ...
  return map(renderContents, (comp, index) => <React.Fragment key={index}>{comp}</React.Fragment>)
}
  ...
  map(header, (title) => {
    switch (title.type) {
      case 'select':
        ...
        break
      case 'date':
        data[title.key] = initData[title.key] ? dayjs(initData[title.key]).toDate() : null
        break
      default:
...
export const validationOneValue = (value, type = null, isRequired, isCheckOnlyEmpty) => {
  if (type === 'checkbox' || type === 'number') {
    // 값이 있느냐 없느냐로 체크하는게 아닌 부분
    switch (type) {
      case 'checkbox':
      ...
      case 'number':
        if (isRequired && (isNil(value) || value === '')) {
          ...
          return {
            success: true,
            value: typeof value === 'string' ? Number(value.replaceAll(/,/g, '')) : value,
          }
        }
    }
  } else {
      ...
      if (value) {
        switch (type) {
          ...
          case 'phone':
            ...
            if (/\d{2,3}-\d{3,4}-\d{4}/g.test(convertPhoneNumber)) {
              return {
                success: true,
                value: convertPhoneNumber.replace(/\s/gi, '').replace(/\-/g, ''),
              }
            } else {
              return {success: false, msg: '연락처를 확인해주세요.'}
            }
          case 'businessNumber':
            const convertBusinessNumber =
              value.indexOf('-') > -1 ? value : makeBusinessNumber(value)
          ...
          case 'ip':
            ...
            if (ipRegExp.test(value)) {
              ...
              return {success: false, msg: 'IP주소를 확인해주세요.'}
            }
          case 'checkboxList':
          ...
          default:
            return {
              success: true,
              value: typeof value === 'string' ? value.replaceAll(/\s/gi, '') : value,
            }
        }
      } else {
        return {success: true, value: value}
...
export const validation = (
  ...
  map(validationTypeList, async (keyData) => {
    ...
    const isCheckOnlyEmpty = keyData?.isCheckOnlyEmpty
    const result = validationOneValue(
      values?.[key],
      validationType || null,
      isRequired,
      isCheckOnlyEmpty,
    ...
    if (result.success) {
      if (hasDuplicateCheck && !duplicateCheck?.[key]) {
        ...
        if (isRemoveNull && (isNil(result.value) || result.value === '')) {
          convertValues[key] = undefined
        } else {
    ...
    if (keyData?.children) {
      ...
      const subIsCheckOnlyEmpty = keyData?.children.isCheckOnlyEmpty
      const subResult = validationOneValue(
        values[subkey],
        subValidationType || null,
        subIsRequired,
        subIsCheckOnlyEmpty,
      )
      ...
      if (subResult.success) {
        if (subHasDuplicateCheck && !duplicateCheck?.[subkey]) {
          ...
          if (isRemoveNull && (isNil(subResult.value) || subResult.value === '')) {
            convertValues[subkey] = undefined
          } else {
...
export const convertSearchDateData = (dateData = null, searchKey = null) => {
  ...
  if (dateData && searchKey) {
    ...
    convertSearchDateData[startKey] = dayjs(dateData.value.start).format('YYYYMMDDHHmm')
    convertSearchDateData[endKey] = dayjs(dateData.value.end).format('YYYYMMDDHHmm')
  }
...
/**
 * timeData.type === 'data' : 날짜데이터를 만드는게 아닌 넘어오는 data 안에서 꺼내 쓰는것으로
 * defaultLength 는 hour 24, day 30, month 12 이 기본값으로 사용됨으로
 * 31일까지 있는 달의 데이터를 만들때는 defaultLength를 31로 넘겨주어야한다
 * @param {Object[]} data
 * @param {Object} timeData {key : string, type: 'hour' | 'day' | 'month' | 'data', defaultLength: number, format:null, unit: undefined}
 * @param {String[]} dataKeys
 */
export const listToGraphData = (data, timeData, dataKeys = [], convertValueFn = null) => {
  ...
  if (timeData.type === 'daily') {
    map(range(0, 24), (_, index) => {
      result.timestamp.push(`${index + 1}${timeData.unit || '시'}`)
  ...
  if (timeData.type === 'monthly') {
    map(range(0, defaultLength || 30), (_, index) => {
      result.timestamp.push(`${index + 1}${timeData.unit || '일'}`)
      map(dataKeys, (dataKey) => {
        result[dataKey].push(null)
      })
    })
  ...
  if (timeData.type === 'yearly') {
    map(range(0, 12), (_, index) => {
      result.timestamp.push(`${index + 1}${timeData.unit || '월'}`)
      map(dataKeys, (dataKey) => {
        result[dataKey].push(null)
      })
    })
  ...
  if (timeData.type === 'data') {
    // 일반 카테고리 데이터
    map(data, (item, index) => {
      result.timestamp.push(
        timeData.format
          ? dayjs(item[timeData.key]).format(timeData.format) + (timeData.unit || '')
          : item[timeData.key] + (timeData.unit || ''),
      )
      map(dataKeys, (dataKey) => {
        const convertValue = convertValueFn
          ? convertValueFn({time: item[timeData.key], value: item[dataKey]}) || null
          : item[dataKey] || null

        result[dataKey].push(convertValue)
      })
    })
  } else {
    // 시간데이터
    map(data, (item) => {
      map(dataKeys, (dataKey) => {
        ...
        result[dataKey][item[timeData.key] - 1] = convertValue
      })
    })
  }
...
export const updateCollection = (targetList, updateItem, primaryKey) => {
  let cloneTargetList = cloneDeep(targetList)
  const equipment = find(cloneTargetList, {[primaryKey]: updateItem[primaryKey]})
  if (equipment) {
    assign(equipment, updateItem)
  } else {
    cloneTargetList = cloneTargetList ? [updateItem].concat(cloneTargetList) : [updateItem]
  }

  return cloneTargetList
}
